...
import {
  X, UserCheck, Phone, Mail, Calendar, MapPin, ShieldAlert, Car, Edit3, Trash2,
  Clock, Route, CheckCircle2, DollarSign, Award, ArrowRight
} from 'lucide-react'
import { liveTrips, formatINR } from '../../data/tripStore'
...
export default function DriverDetailPanel({ driver, isOpen, onClose, onEdit, onDelete, isAdmin }) {
  ...
  // Driver Operational & Financial Analytics from liveTrips
  const driverTrips = liveTrips.filter(t => {
    if (driver.id && t.driverId === driver.id) return true
    if (driver.fullName && t.driverName?.toLowerCase() === driver.fullName.toLowerCase()) return true
    return false
  })

  const totalTrips = driverTrips.length
  const completedTrips = driverTrips.filter(t => t.status === 'Completed').length
  const upcomingTrips = driverTrips.filter(t => ['Booked', 'Confirmed', 'Driver Assigned', 'Vehicle Assigned', 'Started', 'Passenger Picked Up', 'In Transit'].includes(t.status)).length
  const totalRevenue = driverTrips.filter(t => t.status === 'Completed').reduce((sum, t) => sum + (Number(t.actualFare || t.fare) || 0), 0)
  const totalDistance = driverTrips.filter(t => t.status === 'Completed').reduce((sum, t) => sum + (Number(t.estimatedDistance) || 0), 0)
  ...
  return (
    <>
          ...
          <div className="p-5 space-y-6 flex-1">
            ...
            <div className="flex items-center gap-4 rounded-2xl border border-line bg-bg p-4">
              ...
              <div className="space-y-1">
                ...
                {driver.email && <p className="text-[11px] text-ink-soft font-medium">{driver.email}</p>}
              </div>
            </div>

            {/* Operational & Revenue Statistics */}
            <div className="rounded-2xl border border-line bg-surface p-4 space-y-3 shadow-2xs">
              ...
              <div className="grid grid-cols-2 gap-2.5 text-xs">
                <div className="rounded-xl border border-line bg-bg p-3">
                  ...
                  <p className="text-lg font-extrabold text-ink num mt-0.5">{totalTrips}</p>
                </div>

                <div className="rounded-xl border border-line bg-bg p-3">
                  <p className="text-[10px] font-bold text-ink-soft uppercase">Completed</p>
                  <p className="text-lg font-extrabold text-emerald-700 num mt-0.5">{completedTrips}</p>
                </div>

                <div className="rounded-xl border border-line bg-bg p-3">
                  <p className="text-[10px] font-bold text-ink-soft uppercase">Upcoming / Active</p>
                  <p className="text-lg font-extrabold text-sky-700 num mt-0.5">{upcomingTrips}</p>
                </div>

                <div className="rounded-xl border border-line bg-bg p-3">
                  <p className="text-[10px] font-bold text-ink-soft uppercase">Total Distance</p>
                  <p className="text-sm font-extrabold text-ink num mt-0.5">{totalDistance} km</p>
                </div>
              </div>

              <div className="rounded-xl border border-emerald-200 bg-emerald-50/50 p-3 mt-2">
                <p className="text-[10px] font-bold text-emerald-800 uppercase">Associated Trip Revenue</p>
                <p className="text-lg font-extrabold text-emerald-900 num mt-0.5">{formatINR(totalRevenue)}</p>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-line bg-surface p-4 space-y-3 shadow-2xs">
              <div className="flex items-center justify-between border-b border-line pb-2.5">
                <h4 className="text-xs font-bold uppercase tracking-wider text-ink-soft">License & Documents</h4>
            ...
            <div className="rounded-2xl border border-line bg-surface p-4 space-y-3 shadow-2xs">
              <h4 className="text-xs font-bold uppercase tracking-wider text-ink-soft border-b border-line pb-2">
                Driver Trip History ({driverTrips.length})
              </h4>

              {driverTrips.length === 0 ? (
                <p className="text-xs text-ink-soft italic">No trips assigned to this driver yet.</p>
              ) : (
      ...
      {/* Trip Detail Drawer if a trip is tapped */}
      {selectedTrip && (
        <TripDetailPanel
          trip={selectedTrip}
