...
import { editVehicle, normaliseReg, liveVehicles } from '../../data/vehicleStore'
import { liveDrivers } from '../../data/driverStore'

export default function EditVehicleModal({ isOpen, vehicle, onClose, onSuccess }) {
  ...
  useEffect(() => {
    if (isOpen && vehicle) {
      ...
      setInsurancePolicy(vehicle.insurancePolicy || '')
      ...
      setNextServiceOdometer(vehicle.nextServiceOdometer || '')
    }
  }, [isOpen, vehicle])
  ...
  const validate = () => {
    ...
    const isDup = liveVehicles.some(v => v.id !== vehicle.id && normaliseReg(v.reg) === norm)
    if (isDup) {
      return `Registration Number "${reg.trim().toUpperCase()}" is already in use by another vehicle.`
    }
    return null
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (onSuccess) onSuccess('Vehicle updated successfully!')
      onClose()
    } catch (err) {
      ...
      <div className="my-8 w-full max-w-2xl rounded-2xl border border-line bg-surface p-6 shadow-pop animate-scaleUp space-y-5 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between border-b border-line pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-extrabold text-ink">Edit Vehicle Profile</h3>
              <p className="text-xs text-ink-soft">Update fleet specs, document expiries and driver assignment.</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 rounded-xl border border-line bg-bg p-3.5">
              <div className="relative flex h-16 w-24 shrink-0 items-center justify-center rounded-xl border border-line bg-surface overflow-hidden shadow-2xs">
                {photoPreview ? (
                  ...
                  <span className="text-xl font-extrabold text-primary">{name.charAt(0).toUpperCase() || 'V'}</span>
                )}
              </div>

              <div className="flex-1 space-y-1 text-center sm:text-left">
                <label className="text-xs font-bold text-ink">Vehicle Photo</label>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              {/* Vehicle Type */}
              <div className="space-y-1">
                <label className="text-xs font-bold text-ink">Vehicle Type</label>
                <select
                  value={type}
