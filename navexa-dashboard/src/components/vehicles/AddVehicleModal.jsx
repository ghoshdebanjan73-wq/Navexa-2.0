...
import { addVehicle, findVehicleByReg } from '../../data/vehicleStore'
...
export default function AddVehicleModal({ onClose, onSaved, user }) {
  ...
  const validate = () => {
    ...
    if (!reg.trim()) {
      ...
      const existing = findVehicleByReg(reg)
      if (existing) {
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      ...
      if (onSaved) onSaved('Vehicle added successfully.')
      onClose()
    }, 350)
  }
      ...
      <div className="w-full max-w-lg rounded-2xl border border-line bg-surface shadow-pop max-h-[90vh] flex flex-col animate-scaleUp">
        ...
        <div className="flex items-center justify-between border-b border-line px-5 pt-4 pb-3.5 shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base sm:text-lg font-bold text-ink leading-tight">Add Vehicle</h3>
              <p className="text-[11px] text-ink-soft">Add a vehicle to your fleet.</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto p-5 sm:p-6 flex-1 space-y-4" noValidate>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Regular airport vehicle, AC available, luggage carrier, driver preferences..."
